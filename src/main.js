...
const myBot = ['uef72582e5b8f816a0cb940c94d28f67d'];
const banList = [];//Banned list
var groupList = new Array();//Group list
var vx = {};var midnornama,pesane,kickhim;var waitMsg = "no";//DO NOT CHANGE THIS
const imgArr = ['png','jpg','jpeg','gif','bmp','webp'];//DO NOT CHANGE THIS
var komenTL = "AutoLike by VK SELFBOT\nline://ti/p/~0969245004"; //Comment for timeline
...
\n\n# บางคำสั่งไช้งานได้แค่แอดมิน!";
    ...
    poll(operation) {
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "Creator: line://ti/p/~0969245004 (VK SELFBOT)";
			this._client.sendMessage(0, halo);
        }
    ...
    async textMessage(textMessages, seq, param, lockt) {
		...
		if(vx[1] == "ส่งคท" && seq.from_ == vx[0] && waitMsg == "yes"){
			...
			if(txt == "cancel"){
				...
				this._sendMessage(seq,"แทคด้วยค่าาาา!");
			}
		}
		if(txt == "ส่งคท" && !isBanned(banList, seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;vx[2] = "arg1";
			    this._sendMessage(seq,"พิมใหม่ แล้วแทคด้วยค่าา");
			}else{
				waitMsg = "no";vx[0] = "";vx[1] = "";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
			}
		}else if(txt == 'ส่งคท' && isBanned(banList, seq.from_)){this._sendMessage(seq,"ผิดพลาดด!");}
		
		if(vx[1] == "แอดคท" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"# CANCELLED");
		...
		if(txt == "แอดคท" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;vx[2] = "arg1";
		...
		}else if(txt == 'แอดคท' && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		
		if(vx[1] == "เชคไอดี" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"# CANCELLED");
		...
		if(txt == "เชคไอดี" && !isBanned(banList, seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;vx[2] = "arg1";
		...
		}else if(txt == 'เชคไอดี' && isBanned(banList, seq.from_)){this._sendMessage(seq,"Not permitted !");}
		
		if(vx[1] == "ข้อมูล" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"# CANCELLED");
...
vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
		...
		if(txt == "ข้อมูล" && !isBanned(banList, seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;vx[2] = "arg1";
		...
		}else if(txt == 'ข้อมูล' && isBanned(banList, seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		...
		if(vx[1] == "แบน" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		if(txt == "แบน" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
		...
		}else if(txt == "แบน" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		
		if(vx[1] == "แอดมิน" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
		...
		if(txt == "แอดมิน" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
				vx[2] = "arg1";
		...
		}else if(txt == "แอดมิน" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Not permitted !");}
		...
		if(vx[1] == "ล้างดำ" && seq.from_ == vx[0] && waitMsg == "yes"){
			let panjang = txt.split("");
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		if(txt == "ล้างดำ" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
			    waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
				seq.text = "";
		...
		}else if(txt == "ล้างดำ" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาดด !");}
		...
		if(txt == "ออก" && isAdminOrBot(seq.from_)){
			this._client.leaveGroup(0,seq.to);
		}
		
		if(vx[1] == "ยูทูป" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
...
Link Download: "+idU.id+"\n";
		...
		}else if(txt == "ยูทูป" && isBanned(seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
...
\n";
		...
		if(vx[1] == "เพลง" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
			}else if(vx[2] == "arg1" && linktxt[1]){
		...
		if(txt == "เพลง" && !isBanned(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
		...
		}else if(txt == "เพลง" && isBanned(seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		
		if(vx[1] == "บอทออก" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(txt == "cancel"){
				vx[0] = "";vx[1] = "";waitMsg = "no";vx[2] = "";vx[3] = "";
				this._sendMessage(seq,"#CANCELLED");
		...
		if(txt == "บอทออก" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
		...
		}else if(txt == "บอทออก" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
        ...
        }else if(txt == "ยกเลิก" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"คุณไม่ไช่แอดมิน");}

        if(txt == 'สวัสดี') {
			let { mid, displayName } = await this._client.getProfile();
            this._sendMessage(seq, 'สวัสดีค่าาาา >_< '+displayName);
        }
		
		if(vx[1] == "กลุ่ม" && seq.from_ == vx[0] && waitMsg == "yes"){
			if(vx[2]=="arg1"){
			let M = new Message();
		...
		if(txt == "กลุ่ม" && isAdminOrBot(seq.from_)){
			if(vx[2] == null || typeof vx[2] === "undefined" || !vx[2]){
				waitMsg = "yes";
			    vx[0] = seq.from_;vx[1] = txt;
		...
		}else if(txt == "กลุ่ม" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาดด !");}
		
		if(cox[0] == "ประกาศ" && isAdminOrBot(seq.from_) && cox[1]){
            let listMID = [];
        ...
        }else if(cox[0] == "ประกาศ" && isAdminOrBot(seq.from_) && !cox[1]){this._sendMessage(seq,"# How to broadcast:\nbroadcast yourtexthere");}else if(cox[0] == "broadcast" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาด!");}
		
		if(txt == "เตะเรา" && seq.toType == 2 && !isBanned(banList, seq.from_) && this.stateStatus.kick == 1){
			this._sendMessage(seq,"Ok จัดให้!");
			this._kickMember(seq.to,[seq.from_]);
		}else if(txt == 'เตะเรา' && isBanned(banList, seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		
		
		if(txt == "บอทลบแชท" && isAdminOrBot(seq.from_)){
			this._sendMessage(seq, "กำลังลบ message....");
			await this._client.removeAllMessages();
			this._sendMessage(seq, "เสร็จแล้ว..");
		}
		
        const sp = ['!speed','sp','speed','สปีด','respon'];
        if(sp.includes(txt) && !isBanned(banList, seq.from_)) {
            ...
            this._sendMessage(seq, xtime+' วินาที');
        ...
        if(txt == '!speed' && !isBanned(banList, seq.from_)) {
            ...
            const xtime = rtime	- curTime;
            this._sendMessage(seq, xtime+' วินาที');
        }else if(txt == '!speed' && isBanned(banList, seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
        ...
        if(txt === '!หีแตด' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === '!หีแตด' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"ผิดพลาด !");}
...
| บอทไอดี     : \n["+mid+"]\n\
| สถานะบอท : Working\n\
| แอดมินบอท  : "+botOwner[0].displayName+"\n\
====================\n";
		...
		}else if(txt == 'แทค' && isBanned(banList, seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		...
		}else if(cox[0] == "หารูป" && linktxt[1] && isBanned(banList,seq.from_)){this._sendMessage(seq,"Not permitted!");}else if(cox[0] == "!getimage" && !linktxt[1] && !isBanned(banList,seq.from_)){this._sendMessage(seq,"# How to !getimage:\ngetimage http://url.com/image.png");}
		
		if(cox[0] == "album" && isAdminOrBot(seq.from_)){
			await this._createAlbum(seq.to,cox[1],this.config.chanToken);
		}
		
		if(txt == "เตะดำ" && isAdminOrBot(seq.from_)){
			for(var i = 0; i < banList.length; i++){
		...
		}else if(txt == "เตะดำ" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"ผิดพลาด !");}
		
		if(txt == "เชคค่า"){
			this.setState(seq,1)
		}
		
        const action = ['เข้ากลุ่มออโต้ on','เข้ากลุ่มออโต้ off','ยกเลิก on','ยกเลิก off','เตะ on','เตะ off','ต้อนรับ on','ต้อนรับ off','ป้องกัน off','ป้องกัน on','ลิ้ง on','ลิ้ง off']
        if(action.includes(txt)) {
            this.setState(seq,0)
        }
	
        if(txt == 'ไอดี' /*|| txt == 'mid' || txt == 'id'*/) {
            this._sendMessage(seq,"ไอดี: "+seq.from_);
        }
       ...
       /* if(txt == 'speedtest' && isAdminOrBot(seq.from)) {
		...
		if(txt == "เวลา" && !isBanned(banList,seq.from_)){
...
\n# Group Cover:\nhttp://dl.profile.line.naver.jp/"+gcover;
            this._client.sendMessage(0,bang);
        }else if(txt == 'ข้อมูลกลุ่ม' && isBanned(banList, seq.from_)){this._sendMessage(seq,"ผิดพลาดด !");}

        const joinByUrl = ['ลิ้ง','!curl'];
